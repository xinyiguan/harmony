...
from ms3_package import Parse, make_gantt_data, transform, fifths2name, midi2name, name2fifths, name2pc, resolve_dir
...
def create_modulation_plan(data, task_column='semitones', sort_and_fill=True, title='Modulation plan', globalkey=None,
                           phraseends=None, cadences=None, colors=None):
    if sort_and_fill:
...
def create_gantt(data, task_column='Task', title='Gantt chart', colors=None, layout=None, shapes=None, annotations=None,
                 **kwargs):
...
def write_gantt_charts(args):
    ...
    N = len(ats)
    if N == 0:
        p.logger.info("None of the scores contains DCML musana labels, no gantt charts created.")
        return
    else:
    ...
    for (key, i, _), at in ats.items():  # at stands for annotation table, i.e. DataFrame of expanded labels
        ...
        fig = create_modulation_plan(data, title=f"{fname}", globalkey=globalkey, task_column=args.yaxis,
                                     phraseends=phrases)
