...
@dataclass
class PieceInfo:
    piece_path: str  # up to the fname

    def __post_init__(self):
        self.piece_harmonies_tsv_path = self.piece_path + '.tsv'
        self.piece_harmonies_df = self.get_piecewise_harmonies_df()

    def get_piecewise_harmonies_df(self) -> pd.DataFrame:
        all_df = pd.read_csv(self.piece_harmonies_tsv_path, sep='\t')
        return all_df

    def compute_piece_key_probs(self, key: str) -> pd.DataFrame:
        data_df = self.piece_harmonies_df['numeral']
        data_count = data_df.value_counts()
        probs = data_count / data_count.sum()
        return probs

    def compute_piece_key_entropy(self, key: str, base: int = 2) -> float:
        probs = self.compute_piece_key_probs(key=key)
...
class CorpusInfo:
    ...
    def __post_init__(self):
        ...
        self.piece_harmonies_tsv_paths = [f for f in os.listdir(self.corpus_path + 'harmonies/') if
                                          not f.startswith('.') if
                                          not f.startswith('__')]
        self.piece_names_list = [f.replace('.tsv', '') for f in self.piece_harmonies_tsv_paths]
    ...
    def get_pieces_harmonies_df(self, piece_name: str) -> pd.DataFrame:
        piece = PieceInfo(piece_name)
        df = piece.piece_harmonies_df
        return df
    ...
    def compute_piecewise_entropy(self, key: str) -> pd.DataFrame:

        piece_wise_entropy_list = []
        piece_names_list = []
        for idx, val in enumerate(self.piece_names_list):
            piece = PieceInfo(self.corpus_path + 'harmonies/' + val)
            piece_entropy = piece.compute_piece_key_entropy(key=key)
            piece_wise_entropy_list.append(piece_entropy)
            piece_names_list.append(val)

        df_entropy = pd.DataFrame(piece_wise_entropy_list, columns=['entropy'])
        df_names = pd.DataFrame(piece_names_list, columns=['fname'])
        df = pd.concat([df_names, df_entropy], axis=1)
        return df
...
class MetaCorporaInfo:
    ...
    def get_sorted_subcorpus_by_year(self, output_format: typing.Literal['year_corpus_df', 'corpus', 'years']) -> \
            typing.Union[np.array, pd.DataFrame]:
        mean_comp_years = []
    ...
    def get_sns_plot_data(self, key: str):
        """
        columns = ['corpus_name', 'fnames', key, entropies]
        :return:
        """

        corpus_names = self.corpora_harmonies_meta_df['corpus'].values
        df_corpus_name = pd.DataFrame(corpus_names, columns=['corpus'])

        fnames = self.corpora_harmonies_meta_df['fnames'].values
        df_fnames = pd.DataFrame(fnames, columns=['fname'])

        key = self.corpora_harmonies_meta_df['numeral'].values
        pieces_entropies = []
        for idx, val in enumerate(self.subcorpus_paths):
            subcorpus = CorpusInfo(val)
            subcropus_piecewise_entropy = subcorpus.compute_piecewise_entropy(key=key)
            pieces_entropies.append(subcropus_piecewise_entropy)

        df_piece_entropies = pd.concat(pieces_entropies,ignore_index=True)
        df_key = pd.DataFrame(fnames, columns=['numeral'])

        df = pd.concat([df_corpus_name, df_fnames, df_key, df_piece_entropies], axis=1, ignore_index=True)
...
if __name__ == '__main__':
    ...
    # mycorpus_path = 'romantic_piano_corpus/debussy_suite_bergamasque/'
    # mycorpus = CorpusInfo(mycorpus_path)

    print(metacorpora.get_sns_plot_data(key='numeral'))

    # corpus_name =metacorpora.corpora_harmonies_meta_df['corpus'].values
    # print(corpus_name)
