...
class ChordTransitionAnalysis:
    ...
    def plot_to_folder(self, folder_path: str) -> None:
        ...
        self.plot_metacorpora_chord_transitions_heatmap().savefig(fname=f'{folder_path}metacorpora_chord_transitions',
                                                                  dpi=dpi)
        self.plot_chord_transition_in_by_era_heatmap(eras=eras).savefig(fname=f'{folder_path}chord_transitions_by_era',
                                                                        dpi=dpi)

    def plot_metacorpora_chord_transitions_heatmap(self) -> plt.Figure:
        modes = ['M', 'm']
        ...
        for mode, ax in zip(modes, axs):
            ...
            self.transition_matrix_heatmap(transition_matrix=transition_matrix, ax=ax, cmap=cmap)
        return fig

    def plot_chord_transition_in_by_era_heatmap(self, eras: List[str]) -> plt.Figure:
    ...
    def transition_matrix_heatmap(self, transition_matrix: pd.DataFrame, ax: plt.Axes, view_top_n: int = 30,
    ...
    def get_tonal_harmony_transition_in_segment(self, pieceinfos: List[PieceInfo], mode: typing.Literal['M', 'm'],
        ...
        tonal_harmony_transitions = self.tonal_harmony_transitions_from_pieceinfo_list(pieceinfos=pieceinfos,
    ...
    @staticmethod
    def tonal_harmony_transitions_from_pieceinfo_list(pieceinfos: List[PieceInfo], harmony_condition: HarmonyCondition,
...
if __name__ == '__main__':
    ...
    chord_transition_analysis.plot_metacorpora_chord_transitions_heatmap()
